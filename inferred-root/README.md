...
This sub-workflow generates a **Static Inferred Ancestor** for Coxsackievirus A16 (CVA16) using outgroup rooting. This ancestral sequence represents the MRCA (most recent common ancestor) of all CVA16 sequences and serves as a stable, biologically accurate reference for the Nextclade dataset.
...
|-----------|---------|-------------|
| `REFERENCE_ACCESSION` | `"U05876"` | GenBank accession for the reference (G-10) |
...
- Closely related enteroviruses that root the CVA16 tree
...
Expected: Should be ~7,400 bp (full CVA16 genome) with ID `ancestral_sequence`
...
- CVA16 sequences form a monophyletic clade
...
**Solution:** Ensure outgroup sequences are valid FASTA files and compatible with CVA16 (same genomic region). Outgroups should be closely related enteroviruses.
